(function (global, factory) {
    ...
    var BACKSPACE = 8,
        ...
        RIGHT = 39,
        DELETE = 46,
        COMMA = 186, // changesd it from 188 to 186
        ESCAPE = 27;
    var SEPERATOR = ':'; // changed to colon
        ...
        base.input.addEventListener('keydown', function (e) {
          ...
          /*   if (key === ENTER || key === COMMA || key === TAB) { */
            if ( key === COMMA || key === TAB) { 
               ...
               input.dispatchEvent(new CustomEvent("EscapePressed",{detail:null})); // added
            } else if (key ===  ENTER ) {
                input.dispatchEvent(new CustomEvent("EnterPressed",{detail:null}));  // added
            } else {
                return select();
            }
            e.preventDefault();
